...
function getDayName(date) {
  const numberWeekDay = new Date(date).getUTCDay();
  const weekDay = {
    0: 'Sunday',
    1: 'Monday',
    2: 'Tuesday',
...
function getNextFriday(date) {
  ...
  const numberWeekDay = newDate.getUTCDay();
  const nextFriday = (5 - numberWeekDay + 7) % 7 || 7;
  return new Date(newDate.setDate(newDate.getDate() + nextFriday));
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round((newDateEnd - newDateStart) / (1000 * 60 * 60 * 24)) + 1;
}
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hours}:${minute}:${second} ${pmAm}`;
}
...
function getWeekNumberByDate(date) {
  const newDate = new Date(date);
  const weekDay = (newDate.getDay() + 6) % 7;
  const thursday = new Date(newDate);
  thursday.setDate(newDate.getDate() - weekDay + 3);
  ...
  const differentTime = thursday.getTime() - firstThursday.getTime();
  return Math.floor(differentTime / (7 * 1000 * 60 * 60 * 24)) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  nextDate.setMonth(nextDate.getMonth() + 1, 13);
  return nextDate.getDay() === 5 ? nextDate : getNextFridayThe13th(nextDate);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const parseDate = (dateRevert) => {
    const [day, month, year] = dateRevert.split('-').map(Number);
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDate > endDate) return schedule;
